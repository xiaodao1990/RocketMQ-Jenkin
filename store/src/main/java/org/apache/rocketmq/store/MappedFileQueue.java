...
public class MappedFileQueue {
     ...
     * 找到第一个最后一次更新时间大于待查找时间戳的文件，如果不存在，则返回最后一个MappedFile文件
     * @param timestamp
     * @return
     */
    ...
    /**
     * 获取存储文件的最大偏移量。返回最后一个MappedFile文件的fileFromOffset加上MappedFile文件的当前的读指针
     * @return
     */
    ...
    /**
     * 返回存储文件当前的写指针。返回最后一个MappedFile文件的fileFromOffset加上当前写指针位置。
     * @return
     */
     ...
     *
     * 根据offset查找MappedFile直接使用offset%mappedFileSize是否可行？
     *      答案是否定的，由于使用了内存映射，只要存在于存储目录下的文件，都需要对应创建内存映射文件，
     *      如果不定时将已消费的消息从存储文件中删除，会造成极大的内存压力与资源浪费，所以RocketMQ采用
     *      定时删除存储文件的策略，也就是说在存储文件中，第一个文件不一定是00000000000000000000，因为该
     *      文件在某一时刻会被删除，故根据offset定位MappedFile的算法为
