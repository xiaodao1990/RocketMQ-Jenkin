...
public class CommitLog {
    ...
    protected static int calMsgLength(int bodyLength, int topicLength, int propertiesLength) {
        final int msgLen = 4 //TOTALSIZE：该消息条目的总长度
            ...
            + 4 + (bodyLength > 0 ? bodyLength : 0) //BODY：消息体内容，长度为bodyLength中存储的值。bodyLength：消息体长度，4字节
            + 1 + topicLength //TOPIC：主题，长度为topicLength中存储的值。主题存储长度，1字节，表示主题名称不能超过127字节。
            + 2 + (propertiesLength > 0 ? propertiesLength : 0) //properties：消息属性，长度为propertiesLength中存储的值。propertiesLength：消息属性长度，2字节，表四消息属性长度不能超过32767个字符。
            + 0;
        return msgLen;
    }
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            ...
            // 如果mappedFile为空，则创建，用偏移量0创建第一个commit文件，文件名为00000000000000000000。
            // 如果文件创建失败，则抛出CREATE_MAPEDFILE_FAILED异常，很有可能是磁盘空间不足或者权限不足。
            // mappedFileQueue可以看作是${ROCKET_HOME}/store/commitlog文件夹
            // 而mappedFile则对应该文件夹下一个个文件
        ...
        /**
         * DefaultAppendMessageCallback#doAppend只是将消息追加在内存中，需要根据是同步刷盘还是异步刷盘方式，将内存中的
         * 数据持久化到磁盘，然后执行HA主从同步复制
         */
        // 刷盘
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // 将消息内容存储到ButeBuffer中，然后创建AppendMessageResult。这里只是将消息存储在MappedFile对应的内存映射Buffer中
            // 并没有刷写到磁盘，追加结果
            final long beginTimeMills = CommitLog.this.defaultMessageStore.now();
